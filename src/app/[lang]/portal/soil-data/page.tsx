...
export default function SoilMeasurementsPage() {
  // ── Auth + i18n
  ...
  // ── 1) Fetch only this user’s lands
  const {
    data: landsData,
  ...
  // ── 2) Fetch soil measurements (once a land is selected)
  const {
    data: measData,
    loading: measLoading,
    error: measError,
  ...
  /* ── keep a stable array reference for ESLint happiness ─────────────── */
  ...
  // ── Redirect + default‑pick first land
  useEffect(() => {
  ...
  useEffect(() => {
    if (landsData?.lands?.length && !landId) {
      setLandId(landsData.lands[0].id);
      setPage(0);
    }
  }, [landsData, landId]);

  // ── Loading & errors
  if (status === 'loading' || landsLoading) {
    return <p className="p-6">{t[lang].loading}</p>;
  }
  if (landsError) {
    return (
      <p className="p‑6 text‑red‑600">
        {t[lang].error}: {landsError.message}
      </p>
    );
  }
  if (measLoading) {
  ...
  if (measError) {
    return (
      <p className="p‑6 text‑red‑600">
        {t[lang].error}: {measError.message}
      </p>
    );
  }
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      >
        ...
        <div className="md:col-span‑1">
          ...
          <select
            value={landId}
            onChange={e => { setLandId(e.target.value); setPage(0); }}
            className="w-full border rounded p-2"
          >
            {landsData.lands.map((l: unknown) => (
          ...
          return (
            ...
            >
              <button
                ...
                className="absolute top-2 right-2 text-2xl p-2 text-gray-500 hover:text-gray-700 cursor-pointer"
              ...
              <h3 className="text-center font-semibold mb-2">
                {m.label} {landsData.lands.find((l: unknown) => l.id === landId)?.name
                  ? `(${landsData.lands.find((l: unknown) => l.id === landId)!.name})`
                  : ''}
              </h3>
          ...
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-2 text-right">
                {t[lang].organicMatterLabel}
              </th>
              <th className="px-4 py-2 text-right">
                {t[lang].nitrogenLabel}
              </th>
              <th className="px-4 py-2 text-right">
                {t[lang].moistureLabel}
              </th>
              <th className="px-4 py-2 text-right">
                {t[lang].temperatureLabel}
              </th>
            </tr>
          </thead>
          <tbody>
            {measurements.map((m: unknown) => (
              <tr key={m.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-2 text-right">
                  {m.organicMatter?.toFixed(2) ?? '—'}
                </td>
                <td className="px-4 py-2 text-right">
                  {m.nitrogen?.toFixed(2) ?? '—'}
                </td>
                <td className="px-4 py-2 text-right">
                  {m.moisture?.toFixed(2) ?? '—'}
                </td>
                <td className="px-4 py-2 text-right">
